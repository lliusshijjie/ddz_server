...
class LoginService {
public:
    LoginService(SessionManager& session_manager, PlayerManager& player_manager)
        : session_manager_(session_manager), player_manager_(player_manager) {}

    LoginResult HandleLogin(int64_t connection_id, const std::string& request_body, int64_t now_ms);

private:
    static std::optional<int64_t> ParsePlayerIdFromToken(const std::string& token);
